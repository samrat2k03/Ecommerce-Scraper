  ...
  const searchProduct = async () => {
    try {
      ...
      const [amazonResponse, flipkartResponse] = await Promise.all([
        ...
        // new flipkart api
        axios.get(
          `http://127.0.0.1:5000/search/flipkart-new/${isProductSearch}`
        ),
      ]);
  ...
  return (
    <div>
      ...
      <div className="text-center pt-5 text-white">
        <h1 className="text-6xl font-bold p-5 text-white font-poppins">
          Ecommerce Scraper
        </h1>
        ...
        <div>
          {isProductSearch && (
            <p className="text-center font-poppins text-xl pt-[3%] text-white font-semibold">
              Searched Product:{" "}
              <span className="text-red-300">{isProductSearch}</span>
            </p>
          )}
          <div className="flex justify-center mt-8">
            <table className="border-collapse border border-gray-400 bg-white rounded-[10px] w-[90%]">
              ...
              <tbody>
                {[
                  ...Array(
                    Math.max(amazonResult.length, flipkartResult.length)
                  ).keys(),
                ].map((index) => (
                  <tr key={index}>
                      ...
                      {amazonResult[index] && (
                        <div className="flex flex-col items-center gap-3">
                          <p className="font-poppins">
                            ...
                            {amazonResult[index].title}
                          </p>
                          <p className="font-poppins">
                            <span className="text-red-500 pr-2">Price:</span>₹
                            {amazonResult[index].price}
                          </p>
                          <img
                            className="w-[80%]"
                            src={amazonResult[index].image}
                            alt={amazonResult[index].title}
                          />
                          <p className="font-poppins">
                            <span className="text-red-500 px-2">Ratings :</span>
                            {amazonResult[index].rating}
                          </p>
                          <a href={amazonResult[index].link}>
                            <button className="bg-black px-1.5 py-1.5 rounded-xl text-white hover:text-black hover:bg-white">
                              Go to site
                            </button>
                          </a>
                        </div>
                    ...
                    {/* <td className="border border-gray-400 px-4 py-2"> */}
                      {flipkartResult[index] && (
                        <div className="flex flex-col items-center gap-3">
                          <p className="font-poppins">
                            <span className="text-red-500 pr-2">Title:</span>
                            {flipkartResult[index].result.name}
                          </p>
                          <p className="font-poppins">
                            <span className="text-red-500 pr-2">Price:</span>
                            {flipkartResult[index].result.original_price}
                          </p>
                          <img
                            className="w-[40%]"
                            src={flipkartResult[index].result.thumbnail}
                            alt={flipkartResult[index].result.name}
                          />
                          <p className="font-poppins">
                            <span className="text-red-500 px-2">Ratings :</span>
                            {/* {flipkartResult[index].rating} */}
                          </p>
                          <a href={flipkartResult[index].result.query_url}>
                            <button className="bg-black px-1.5 py-1.5 rounded-xl text-white hover:text-black hover:bg-white">
                              Go to site
                            </button>
                          </a>
